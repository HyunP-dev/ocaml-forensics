...
CAMLprim value caml_get_name_from_value(value v_value)
{
    CAMLparam1(v_value);
    CAMLlocal1(v_result);

    // TODO: 메모리가 도중에 해제 되는 모양.
    libregf_value_t *value_ = (libregf_value_t *)Nativeint_val(v_value);
    libregf_error_t *error = NULL;

    size_t name_size;

    if (libregf_value_get_name_size(value_, &name_size, &error) != 1)
    {
        goto error;
    }
    ...
    v_result = caml_alloc(1, 0);
    Store_field(v_result, 0, caml_copy_string((const char *)name));
    CAMLreturn(v_result);
