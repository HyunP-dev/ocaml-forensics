...
CAMLprim value caml_get_subkey_names(value v_key)
{
    ...
    if (libregf_key_get_number_of_sub_keys(key, &number_of_sub_keys, &error) != 1) goto error;

    {
        ...
        for (int i = 0; i < number_of_sub_keys; i++)
        {
            ...
            if (libregf_key_get_sub_key_by_index(key, i, &subkey, &error) != 1) goto error;

            size_t name_size;
            if (libregf_key_get_utf8_name_size(subkey, &name_size, &error) != 1) goto error;

            char *name = malloc(name_size);

            if (libregf_key_get_utf8_name(subkey, (uint8_t *)name, name_size, &error) != 1) goto error;
...
CAMLprim value caml_get_values(value v_key)
{
    ...
    if (libregf_key_get_number_of_values(key, &number_of_values, &error) != 1) goto error;

    {
        ...
        for (int i = 0; i < number_of_values; i++)
        {
            ...
            if (libregf_key_get_value_by_index(key, i, &value_, &error) != 1) goto error;
...
CAMLprim value caml_get_value_names(value v_key)
{
...

    {
        ...
        for (int i = 0; i < number_of_values; i++)
        {
            ...
            size_t name_size;
            if (libregf_value_get_utf8_name_size(value_, &name_size, &error) != 1) goto error;

            char *name = malloc(name_size);
            if (libregf_value_get_utf8_name(value_, (uint8_t *)name, name_size, &error) != 1) goto error;
...
CAMLprim value caml_get_value(value v_key, value v_value_name)
{
    ...
    CAMLlocal4(v_result, v_value_data, v_tagged_value, v_value_type);
    ...
    libregf_error_t *error = NULL;
    libregf_value_t *value_ = NULL;
    if (libregf_key_get_value_by_utf8_name(key, (const uint8_t *)value_name, strlen(value_name), &value_, &error) != 1) goto error;

    size_t data_size;
    if (libregf_value_get_value_data_size(value_, &data_size, &error) != 1) goto error;

    uint8_t *value_data = malloc(data_size);
    if (libregf_value_get_value_data(value_, value_data, data_size, &error) != 1) goto error;

    uint32_t value_type;
    if (libregf_value_get_value_type(value_, &value_type, &error) != 1) goto error;

    v_result = caml_alloc(1, 0);
    v_value_data = caml_alloc_string(data_size);
    memcpy((void *)Bytes_val(v_value_data), (const void *)value_data, data_size);

    v_tagged_value = caml_alloc(2, 0);
    v_value_type = Val_int(value_type);
    Store_field(v_tagged_value, 0, v_value_type);
    Store_field(v_tagged_value, 1, v_value_data);

    Store_field(v_result, 0, v_tagged_value);
    CAMLreturn(v_result);
